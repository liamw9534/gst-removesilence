...
gst_remove_silence_class_init (GstRemoveSilenceClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_GATE,
      g_param_spec_boolean ("gate", "Gate",
          "Set to true to drop all samples but still track the silence state",
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_GATE:
      filter->gate = g_value_get_boolean (value);
      break;
    case PROP_REMOVE:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_GATE:
      g_value_set_boolean (value, filter->gate);
      break;
    case PROP_REMOVE:
...
gst_remove_silence_transform_ip (GstBaseTransform * trans, GstBuffer * inbuf)
{
    ...
    if (!filter->silence_detected && consecutive_silence_reached) {
      if (!filter->silent) {
        if (GST_BUFFER_PTS_IS_VALID (inbuf)) {
          ...
          GST_DEBUG ("*** Posting silence_detected");
      ...
      if (!filter->silent) {
        if (GST_BUFFER_PTS_IS_VALID (inbuf)) {
          ...
          s = gst_structure_new ("removesilence", "silence_finished",
              G_TYPE_UINT64, GST_BUFFER_PTS (inbuf) - filter->ts_offset, NULL);
          m = gst_message_new_element (GST_OBJECT (filter), s);
          GST_DEBUG ("*** Posting silence_finished");
          gst_element_post_message (GST_ELEMENT (filter), m);
        }
      }
      filter->silence_detected = FALSE;
  ...
  return filter->gate ? GST_BASE_TRANSFORM_FLOW_DROPPED : GST_FLOW_OK;
}
